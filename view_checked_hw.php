...
$sid = $_SESSION['tid'];
$query = mysqli_query($con,"SELECT step.name,home.hwch FROM home INNER JOIN step ON home.tid = step.id where home.sid = '$sid' && hwch IS NOT NULL order by step.name asc");
...
<head profile="pic.jpg">
	<title>View checked Homeworks sent by teachers for free with peakme</title>
	<meta charset="utf-8">
...
<table class="vietab"><tr><th>S. no.</th><th>Name</th><th>Files</th></tr>
        <?php
        if(mysqli_num_rows($query)>0){
            ...
            while($imp=mysqli_fetch_array($query)){
                ...
                if(count($array)==2){
                    $d = strrpos("$imp[1]", ",");
                    $goodUrl = $imp[1];
                    if($d){$goodUrl = str_replace(',', '', $imp[1]);
                    $anc = "<a href='chhw/$goodUrl'>Files</a>";
                }}else{
                    $anc = "<a onclick='down(this.id)' style='color:blue;' id='$imp[1]'>Files</a>";
                }
                echo "<tr><td>$a</td><td>$imp[0]</td><td>$anc</td></tr>";
            }}
        ?>
        </table>
        <script>
            function down(click){
                ido = click;
                window.location.href = "https://peakme.in/viewtwo.php?checkedhw="+ido;
            }
        </script>
    </div>
</div>
